...
vlog -incr -work work ../../ip/llr_ram.v
...
vlog -incr -work work ../../rtl/cpu_addr_gen.v
vlog -incr -work work ../../rtl/ldpc_cpu.v
vlog -incr -work work ../../rtl/ldpc_runner_top.v
vlog -incr -work work ../../rtl/ldpc_vpu.v
vlog -incr -work work ../../rtl/vpu_addr_gen.v
vlog -incr -work work ../../tb/tb_ldpc_runner_top.v

# 固定格式，虽不知为何一定要用 -voptargs=+acc 但默认就是这种方式
# 不加的话代码跑不下去
vsim -voptargs=+acc work.tb_ldpc_runner_top

# 添加波形
# 这里可以引入更加复杂的设置
add wave -divider tb_ldpc_runner_top
add wave sim:/tb_ldpc_runner_top/*
